...
import com.telamin.fluxtion.builder.flowfunction.*;
...
import com.telamin.fluxtion.runtime.flowfunction.function.NodeToFlowFunction;
import com.telamin.fluxtion.runtime.flowfunction.function.QuinPushFunction;
...
import com.telamin.fluxtion.runtime.node.NamedFeedTopicFilteredEventHandlerNode;
 ...
 * @see FlowFunction
 */
// Browser override: changed from interface to final class so javac emits
// internal synthetic lambda methods at the class level rather than as
// `private static` interface members. The latter are unreachable from JDK 8
// MethodHandles (Java 9 feature), which blocks Retrolambda from reifying
// them. All methods are static utilities; no implementations exist anywhere.
public final class DataFlowBuilder {
    private DataFlowBuilder() {}

    /**
     * Subscribes to events of type {@literal <T>}. Creates a handler method in the generated {@link DataFlow}
     * so that if {@link DataFlow#onEvent(Object)} is called an invocation is routed
     * to this {@link FlowFunction}
     *
     * @param classSubscription A class literal describing the subscription
     * @param <T>               The actual type dispatched to this {@link FlowFunction} by the generated {@link DataFlow}
    ...
    public static <T> FlowBuilder<T> subscribe(Class<T> classSubscription) {
        return new FlowBuilder<>(
                GeneratorNodeCollection.service().addOrReuse(new DefaultEventHandlerNode<>(classSubscription))
        );
    }

    /**
     * Subscribes to events of type {@literal <T>} apply an instance function publishing the return value. Creates a
     * handler method in the generated {@link DataFlow}
     * so that if {@link DataFlow#onEvent(Object)} is called an invocation is routed
     * to this {@link FlowFunction}
     * <p>
     ...
     * @param <T>      The type subscribed to by the generated {@link DataFlow}
     * @param <R>      The output type of the map function applied to input instances
     * @return An {@link FlowBuilder} that can used to construct stream processing logic
     */
    public static <T, R> FlowBuilder<R> subscribe(SerializableFunction<T, R> function) {
        Class<T> classSubscription = (Class<T>) function.method().getDeclaringClass();
        return subscribe(classSubscription).map(function);
    }

    /**
     * Subscribes to {@link NamedFeedEvent}'s published by an {@link EventFeed} service
     * registered with feedName.
     *
     * @param feedName the service name of the registered {@link EventFeed}
     * @return An {@link FlowBuilder} that can used to construct stream processing logic
     */
    public static FlowBuilder<NamedFeedEvent<ByteBuffer>> subscribeToFeed(String feedName) {
    ...
    /**
     * Subscribes to events of type {@link NamedFeedEvent<T>} published by an {@link EventFeed} service
     * registered with feedName.
     *
     * @param feedName the service name of the registered {@link EventFeed}
     ...
     */
    public static FlowBuilder<NamedFeedEvent<ByteBuffer>> subscribeToFeed(String feedName, String topic) {
    ...
    /**
     * Subscribes to events of type {@literal <T>} filtering by {@link Event#filterString()}. Creates a handler method in the generated {@link DataFlow}
     * so that if {@link DataFlow#onEvent(Object)} is called an invocation is routed
     * to this {@link FlowFunction}
     *
     * @param classSubscription A class literal describing the subscription
    ...
    public static <T extends Event> FlowBuilder<T> subscribe(Class<T> classSubscription, String filter) {
        return new FlowBuilder<>(
                GeneratorNodeCollection.service().addOrReuse(new DefaultEventHandlerNode<>(filter, classSubscription))
        );
    }

    /**
     * Subscribes to events of type {@literal <T>} filtering by {@link Event#filterId()}. Creates a handler method in the generated {@link DataFlow}
     * so that if {@link DataFlow#onEvent(Object)} is called an invocation is routed
     * to this {@link FlowFunction}
     *
     * @param classSubscription A class literal describing the subscription
     ...
     * @return An {@link FlowBuilder} that can used to construct stream processing logic
     */
    public static <T, R> FlowBuilder<R> subscribeToNodeProperty(SerializableFunction<T, R> sourceProperty) {
        T source;
     ...
     * @return An {@link FlowBuilder} that can used to construct stream processing logic
     */
    public static <R> FlowBuilder<R> subscribeToNodeProperty(SerializableSupplier<R> propertySupplier) {
        GeneratorNodeCollection.service().addOrReuse(propertySupplier.captured()[0]);
        return new FlowBuilder<>(new com.telamin.fluxtion.runtime.flowfunction.function.NodePropertyToFlowFunction<>(propertySupplier));
    }

    /**
     * See {@link DataFlowBuilder#subscribe(Class)} shortcut method to create a stream that subscribes to a filtered
    ...
    /**
     * See {@link DataFlowBuilder#subscribe(Class)} shortcut method to create a int stream that subscribes to a filtered
     * {@link Signal} event. Useful for invoking triggers on flows.
     *
     * @param filterId The filter string to apply
    ...
    /**
     * See {@link DataFlowBuilder#subscribe(Class)} shortcut method to create a double stream that subscribes to a filtered
     * {@link Signal} event. Useful for invoking triggers on flows.
     *
     * @param filterId The filter string to apply
    ...
    /**
     * See {@link DataFlowBuilder#subscribe(Class)} shortcut method to create a long stream that subscribes to a filtered
     * {@link Signal} event. Useful for invoking triggers on flows.
     *
     * @param filterId The filter string to apply
     ...
     * to a target instance. To create {@link MergeAndMapFlowBuilder.MergeInput}
     ...
     */
    @SafeVarargs
    public static <T> FlowBuilder<T> mergeMap(LambdaReflection.SerializableSupplier<T> target, MergeAndMapFlowBuilder.MergeInput<T, ?>... mergeInputs) {
     ...
     */
    @SafeVarargs
    public static <K, T> FlowBuilder<T> mergeMapToNode(T target, MergeAndMapFlowBuilder.MergeInput<T, ?>... mergeInputs) {
     ...
     * @param streamAToMerge stream A to merge
     * @param streamBToMerge stream B to merge
     * @param streamsToMerge stream to merge
     * @param <T>            type of stream A
     * @param <S>            type of stream B
     * @return An {@link FlowBuilder} that can used to construct stream processing logic
     */
    @SuppressWarnings("unchecked")
    public static <T, S extends T> FlowBuilder<T> merge(
    ...
    /**
     * Applies a mapping bi function to a pair of stream, creating a stream that is the output of the function. The
     ...
     * @return An {@link FlowBuilder} that can used to construct stream processing logic
     */
    public static <T, S, R> FlowBuilder<R> mapBiFunction(SerializableBiFunction<T, S, R> biFunction,
     ...
     */
    public static <T, K> GroupByFlowBuilder<K, T> groupBy(SerializableFunction<T, K> keyFunction) {
        @SuppressWarnings("unchecked")
    ...
    public static <T, K, V> GroupByFlowBuilder<K, V> groupByFromMap(SerializableFunction<T, Map<K, V>> mapSupplier) {
        // Browser override: explicit lambda -> REF_invokeStatic synthetic body.
        GroupByHashMap<K, V> hashMap = new GroupByHashMap<>();
        MapRef2RefFlowFunction<Map<K, V>, GroupBy<K, V>, TriggeredFlowFunction<Map<K, V>>> triggered = new MapRef2RefFlowFunction<>(
                subscribe(mapSupplier).getEventStream(),
                m -> hashMap.fromMap(m)
        );
    ...
    @SafeVarargs
    public static <T> GroupByFlowBuilder<GroupByKey<T>, T> groupByFields(SerializableFunction<T, ?>... keyFunction) {
        @SuppressWarnings("unchecked")
     ...
     * @return A GroupByFlowBuilder for further configuration
     */
    public static <T, K, O, F extends AggregateFlowFunction<T, O, F>> GroupByFlowBuilder<K, O> groupBy(
     ...
     */
    public static <T, K> GroupByFlowBuilder<K, List<T>> groupByToList(SerializableFunction<T, K> keyFunction) {
        @SuppressWarnings("unchecked")
        Class<T> classSubscription = (Class<T>) keyFunction.method().getDeclaringClass();
     ...
     */
    public static <T> FlowBuilder<Collection<T>> collectionFromSubscribe(Class<T> classSubscription) {
     ...
     */
    public static <T> FlowBuilder<List<T>> listFromSubscribe(Class<T> classSubscription) {
     ...
     */
    public static <T> FlowBuilder<Set<T>> setFromSubscribe(Class<T> classSubscription) {
     ...
     */
    @SafeVarargs
    public static <T> GroupByFlowBuilder<GroupByKey<T>, List<T>> groupByToList(LambdaReflection.SerializableFunction<T, ?>... keyFunctions) {
        @SuppressWarnings("unchecked")
     ...
     */
    public static <T, K> GroupByFlowBuilder<K, Set<T>> groupByToSet(SerializableFunction<T, K> keyFunction) {
        @SuppressWarnings("unchecked")
        Class<T> classSubscription = (Class<T>) keyFunction.method().getDeclaringClass();
     ...
     */
    @SafeVarargs
    public static <T> GroupByFlowBuilder<GroupByKey<T>, Set<T>> groupByToSet(LambdaReflection.SerializableFunction<T, ?>... keyFunctions) {
        @SuppressWarnings("unchecked")
        Class<T> classSubscription = (Class<T>) keyFunctions[0].method().getDeclaringClass();
     ...
     * @return A GroupByFlowBuilder for further configuration
     */
    public static <T, K, V> GroupByFlowBuilder<K, V> groupBy(
    ...
    public static <T, V, K1, A, F extends AggregateFlowFunction<V, A, F>> GroupByFlowBuilder<K1, A>
     ...
     */
    public static <T, A, B> FlowBuilder<T> push(
        ...
        com.telamin.fluxtion.runtime.flowfunction.function.BiPushFunction<T, A, B> pushFunction = new com.telamin.fluxtion.runtime.flowfunction.function.BiPushFunction<>(
                instancePushMethod,
     ...
     * @return A {@link FlowBuilder} wrapping an instance of T that can used to construct stream processing logic
     */
    public static <T, A, B, C> FlowBuilder<T> push(
            SerializableTriConsumer<A, B, C> instancePushMethod,
            FlowDataSupplier<? extends FlowSupplier<A>> flowBuilderA,
            FlowDataSupplier<? extends FlowSupplier<B>> flowBuilderB,
            FlowDataSupplier<? extends FlowSupplier<C>> flowBuilderC) {
        com.telamin.fluxtion.runtime.flowfunction.function.TriPushFunction<T, A, B, C> pushFunction = new com.telamin.fluxtion.runtime.flowfunction.function.TriPushFunction<>(
                instancePushMethod,
                flowBuilderA.flowSupplier(),
                flowBuilderB.flowSupplier(),
     ...
     * @return A {@link FlowBuilder} wrapping an instance of T that can used to construct stream processing logic
     */
    public static <T, A, B, C, D> FlowBuilder<T> push(
            SerializableQuadConsumer<A, B, C, D> instancePushMethod,
            FlowDataSupplier<? extends FlowSupplier<A>> flowBuilderA,
            FlowDataSupplier<? extends FlowSupplier<B>> flowBuilderB,
            FlowDataSupplier<? extends FlowSupplier<C>> flowBuilderC,
            FlowDataSupplier<? extends FlowSupplier<D>> flowBuilderD) {
        com.telamin.fluxtion.runtime.flowfunction.function.QuadPushFunction<T, A, B, C, D> pushFunction = new com.telamin.fluxtion.runtime.flowfunction.function.QuadPushFunction<>(
                instancePushMethod,
                flowBuilderA.flowSupplier(),
                flowBuilderB.flowSupplier(),
                flowBuilderC.flowSupplier(),
     ...
     * @return A {@link FlowBuilder} wrapping an instance of T that can used to construct stream processing logic
     */
    public static <T, A, B, C, D, E> FlowBuilder<T> push(
     ...
     * @return A {@link FlowBuilder} wrapping an instance of T that can used to construct stream processing logic
     */
    public static <T, A, B, C, D, E, F> FlowBuilder<T> push(
        ...
        com.telamin.fluxtion.runtime.flowfunction.function.SextPushFunction<T, A, B, C, D, E, F> pushFunction = new com.telamin.fluxtion.runtime.flowfunction.function.SextPushFunction<>(
                instancePushMethod,
                flowBuilderA.flowSupplier(),
                flowBuilderB.flowSupplier(),
                flowBuilderC.flowSupplier(),
