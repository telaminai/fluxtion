...
import com.fluxtion.dataflow.runtime.flowfunction.aggregate.function.primitive.LongSumFlowFunction;
...
import com.fluxtion.dataflow.stream.EventStreamBuildTest.NotifyAndPushTarget;
import com.fluxtion.dataflow.builder.flowfunction.DoubleFlowBuilder;
...
import static com.fluxtion.dataflow.runtime.flowfunction.helpers.Aggregates.countFactory;
...
//        auditToFile("intTest");
    ...
    public void streamMembersTest() {
        sep(c -> c.addNode(new StreamMembers(
                subscribe(Integer.class).mapToInt(Integer::intValue).flowSupplier(),
                subscribe(Double.class).mapToDouble(Double::doubleValue).flowSupplier(),
                subscribe(Long.class).mapToLong(Long::longValue).flowSupplier()
        ), "root"));
...
//                .aggregate(AggregateDoubleSum::new).id("sum")
    ...
    public void tumblingDoubleMap() {
        sep(c -> subscribe(String.class)
                .mapToDouble(EventStreamBuildTest::parseDouble)
                .tumblingAggregate(DoubleSumFlowFunction::new, 300).id("sum")
                .push(new NotifyAndPushTarget()::setDoublePushValue));
        NotifyAndPushTarget notifyTarget = getField(NotifyAndPushTarget.DEFAULT_NAME);
    ...
    public void tumblingLongMap() {
        sep(c -> subscribe(String.class)
                .mapToLong(EventStreamBuildTest::parseLong)
                .tumblingAggregate(LongSumFlowFunction::new, 300).id("sum")
                .push(new NotifyAndPushTarget()::setLongPushValue));
        NotifyAndPushTarget notifyTarget = getField(NotifyAndPushTarget.DEFAULT_NAME);
    ...
    public void slidingLongWindowTest() {
        sep(c -> subscribe(String.class)
                .mapToLong(EventStreamBuildTest::parseLong)
                .slidingAggregate(LongSumFlowFunction::new, 100, 4).id("sum")
                .push(new NotifyAndPushTarget()::setLongPushValue));
        setTime(0);
        onEvent("10");
        onEvent("10");
...
//        addAuditor();
        sep(c -> {
            NotifyAndPushTarget pushTarget = new NotifyAndPushTarget();
            DoubleFlowBuilder doubleStreamBuilder = subscribe(Double.class).mapToDouble(Double::doubleValue);
...
//        addAuditor();
        sep(c -> {
            NotifyAndPushTarget pushTarget = new NotifyAndPushTarget();
            LongFlowBuilder longStreamBuilder = subscribe(Long.class).mapToLong(Long::longValue);
