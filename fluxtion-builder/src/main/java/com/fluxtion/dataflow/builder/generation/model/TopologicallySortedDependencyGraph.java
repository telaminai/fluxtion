...
import com.fluxtion.dataflow.builder.generation.config.RootNodeConfig;
...
public class TopologicallySortedDependencyGraph implements NodeRegistry {
    ...
    private Map<Object, Boolean> addNodeMap;
    private boolean processed = false;
    private final ReflectionCache reflectionCache;
    private final Set<Object> identitySet = Collections.newSetFromMap(new IdentityHashMap<>());
    ...
    public SimpleDirectedGraph<Object, DefaultEdge> getGraph() {
        return (SimpleDirectedGraph<Object, DefaultEdge>) graph.clone();
    }
     ...
     * @param nodeFactoryRegistration node description
     ...
     * @throws Exception when generating graph
     */
        ...
        try {
            ...
            if (handle != null) {
                ...
                try {
                    ...
                    final Mirror constructor = new Mirror();
                        ...
                        .forEach(map -> {
                            ...
                            switch (clazz1.getSimpleName()) {
                                ...
                                case "Character":
                                    ...
                                    break;
                                default:
                                    throw new RuntimeException("Type not supported in default node ");
                            }
                        });
            }
            ...
            LOGGER.error("error creating node with node", ex);
            throw new RuntimeException("error creating node with node", ex);
        }
    }
...
//        Type genericReturnType = createMethod.getGenericReturnType();
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Registered node:" + clazz.getCanonicalName() + " building:" + targetClass);
        }
    ...
    private void walkDependenciesForEventHandling(Object object) throws IllegalArgumentException, IllegalAccessException {
        ...
        @SuppressWarnings("unchecked") Set<Field> s = reflectionCache.getAllFields(clazz);
        Field[] fields = new Field[s.size()];

        @SuppressWarnings("unchecked") boolean overrideEventTrigger = reflectionCache.getAllFields(clazz, TriggerEventOverride.class).stream()
                .anyMatch(f -> {
                    try {
    ...
    private String getInstanceName(Field field, Object node) throws IllegalArgumentException, IllegalAccessException {
        ...
        if (refField != null && field.getAnnotation(ExcludeNode.class) == null) {
            if (addNodeMap.containsKey(refField)) {
                addNode = addNodeMap.get(refField);
            } else {
                ...
                ;
                addNodeMap.put(refField, addNode);
            }
        }
    ...
    private boolean handlesEvents(Object obj) {
        return reflectionCache.isHandleEvent(obj);
    }

    private void walkDependencies(Object object) throws IllegalArgumentException, IllegalAccessException {
        if (object != null && DataFlow.class.isAssignableFrom(object.getClass())) {
            LOGGER.debug("dont walk dependencies for DataFlow field:{}", object);
            return;
        }
        ...
        @SuppressWarnings("unchecked") Set<Field> s = reflectionCache.getAllFields(object.getClass());
        Field[] fields = new Field[s.size()];
        fields = s.toArray(fields);
        for (Field field : fields) {
            if (!trySetAccessible(field) || Modifier.isTransient(field.getModifiers())) {
            ...
            if (injecting != null & refName == null & field.get(object) == null) {
                ...
                Set<Field> fieldNames = ReflectionUtils.getAllFields(object.getClass(), ReflectionUtils.withName(factoryVariableName));
                if (factoryVariableName.length() > 0 && fieldNames.size() > 0) {
                    Field f = fieldNames.iterator().next();
                    f.setAccessible(true);
