...
import com.fluxtion.dataflow.builder.generation.model.CbMethodHandle;
import com.fluxtion.dataflow.builder.generation.model.ExportFunctionData;
...
import com.fluxtion.dataflow.runtime.annotations.AfterTrigger;
...
@Slf4j
public class InMemoryEventProcessor implements CloneableDataFlow, DataFlow, InternalEventProcessor, Lifecycle, BatchHandler {

    private final SimpleEventProcessorModel simpleEventProcessorModel;
    private final EventProcessorConfig config;
    private final MutableDataFlowContext context;
    ...
    public InMemoryEventProcessor(SimpleEventProcessorModel simpleEventProcessorModel, EventProcessorConfig config) {
        ...
        this.config = config;
        try {
            context = getNodeById(DataFlowContext.DEFAULT_NODE_NAME);
...
//        dirtyBitset.clear();
    ...
    public <T> T exportedService(Class<T> exportedServiceClass) {
        if (exportedServiceClass.isAssignableFrom(DataFlow.class)) {
            return (T) this;
        }
...
//        System.out.println(sb.toString());
    ...
    @Data
    private class Node implements DataFlow, Lifecycle {
