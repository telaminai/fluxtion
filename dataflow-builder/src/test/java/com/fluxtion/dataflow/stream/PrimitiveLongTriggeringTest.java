...
import com.fluxtion.dataflow.runtime.flowfunction.aggregate.function.primitive.LongMaxFlowFunction;
...
public class PrimitiveLongTriggeringTest extends MultipleSepTargetInProcessTest {
    ...
    public void resetMapTest() {
        sep(c -> DataFlowBuilder.subscribeToLongSignal("in")
    ...
    public void additionalPublishAggregateTest() {
        sep(c -> DataFlowBuilder.subscribeToLongSignal("in")
                .aggregate(LongSumFlowFunction::new)
                .publishTrigger(DataFlowBuilder.subscribeToSignal("publish"))
                .sink("out"));
    ...
    public void resetTumblingMapTest() {
        sep(c -> DataFlowBuilder.subscribeToLongSignal("in")
                .tumblingAggregate(LongSumFlowFunction::new, 100).id("sum")
                .resetTrigger(DataFlowBuilder.subscribeToSignal("reset"))
                .sink("out"));
    ...
    public void slidingWindowNonDeductTest() {
        sep(c -> subscribe(String.class)
                ...
                .slidingAggregate(LongMaxFlowFunction::new, 100, 4).id("max")
                .resetTrigger(DataFlowBuilder.subscribeToSignal("reset"))
        );
        addClock();
        onEvent("70");
