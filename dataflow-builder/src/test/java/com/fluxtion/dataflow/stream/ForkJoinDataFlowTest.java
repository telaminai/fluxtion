...
import com.fluxtion.dataflow.runtime.audit.LogRecord;
...
//        addAuditor();
        sep(c -> {
            c.addNode(SyncCollectorMulti.builder().name("multiCollector")
                    .parent(
                            DataFlowBuilder.subscribeToSignal("async_1")
...
//        sep.setAuditLogLevel(LogLevel.DEBUG);
    ...
    public static class AsyncProcess implements NamedNode {
        ...
        public AsyncProcess(String name, int wait) {
            this(
                    name + "_" + wait,
                    wait,
                    DataFlowBuilder.subscribeToSignal(name)
                            .map(new MyConverter()::toUpper)
                            .flowSupplier());
        }
        ...
        public boolean trigger() {
            log.debug("trigger::start {}", toString());
            Thread.sleep(waitMillis);
            log.debug("trigger::complete {}", toString());
            return true;
        }
    ...
    public static class MyConverter {
        ...
        public static String toUpperStatic(Object in) {
            log.debug("converting to upper");
            Thread.sleep(25);
            String upperCase = in.toString().toUpperCase();
            log.debug("converted:{}", upperCase);
            return upperCase;
        }
    ...
    public static class SyncCollectorMulti implements NamedNode {
        ...
        @OnParentUpdate
        public void parentUpdated(Object parent) {
            log.debug("parentUpdated:{}", parent instanceof FlowSupplier ? ((FlowSupplier) parent).get() : "--");
        }
