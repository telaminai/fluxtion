...
import static com.fluxtion.dataflow.builder.flowfunction.MultiJoinBuilder.multiJoinLeg;
...
public class MultiJoinTest extends MultipleSepTargetInProcessTest {
    ...
    public void resetJoin() {
        sep(c -> MultiJoinBuilder
                ...
                .addJoin(DataFlowBuilder.groupBy(LeftData::getName), MergedData::setLeftData)
                .addJoin(DataFlowBuilder.groupBy(MiddleData::getName), MergedData::setMiddleData)
                .addJoin(DataFlowBuilder.groupBy(RightData::getName), MergedData::setRightData)
                .dataFlow()
    ...
    public void multiJoinFromHelper() {
        sep(c -> {
            DataFlowBuilder.multiJoin(
                            MergedData::new,
                            multiJoinLeg(DataFlowBuilder.groupBy(LeftData::getName), MergedData::setLeftData),
                            multiJoinLeg(DataFlowBuilder.groupBy(MiddleData::getName), MergedData::setMiddleData),
                            multiJoinLeg(DataFlowBuilder.groupBy(RightData::getName), MergedData::setRightData))
                    .mapValues(MergedData::formattedString)
                    .map(GroupBy::toMap)
                    .id("results");
        });
