...
import com.fluxtion.dataflow.runtime.flowfunction.aggregate.function.AbstractAggregateFlowFunction;
import com.fluxtion.dataflow.runtime.flowfunction.aggregate.function.primitive.DoubleSumFlowFunction;
import com.fluxtion.dataflow.runtime.flowfunction.aggregate.function.primitive.IntSumFlowFunction;
import com.fluxtion.dataflow.runtime.flowfunction.groupby.GroupBy;
import com.fluxtion.dataflow.runtime.flowfunction.groupby.GroupBy.KeyValue;
...
import com.fluxtion.dataflow.stream.EventStreamBuildTest.KeyedData;
import com.fluxtion.dataflow.stream.EventStreamBuildTest.MergedType;
import com.fluxtion.dataflow.stream.EventStreamBuildTest.MyIntFilter;
...
import com.fluxtion.dataflow.builder.flowfunction.GroupByFunction;
...
import static com.fluxtion.dataflow.builder.DataFlowBuilder.*;
...
public class GroupByTest extends MultipleSepTargetInProcessTest {
    ...
    public void groupByIdentityTest() {
        ...
        sep(c -> {
            DataFlowBuilder.subscribe(Data.class)
    ...
    public void dataFlowGroupByAsListIdentityTest() {
        Map<String, List<Data>> expected = new HashMap<>();
        sep(c -> {
            DataFlowBuilder.groupByToList(Data::getName)
                    .map(GroupBy::toMap).id("results");
        });
    ...
    public void dataFlowGroupByAsListMultiFieldKeyIdentityTest() {
        ...
        sep(c -> {
            DataFlowBuilder.groupByToList(Data::getName, Data::getValue)
                    .map(GroupBy::toMap).id("results");
        });
    ...
    public void dataFlowGroupBySet() {
        ...
        sep(c -> {
            DataFlowBuilder.groupByToSet(Data::getName)
                    .map(GroupBy::toMap).id("results");
        });
        onEvent(new Data("A", 25));
        onEvent(new Data("A", 50));
    ...
    public void dataFlowGroupByAsASetMultiFieldKeyIdentityTest() {
        ...
        sep(c -> {
            DataFlowBuilder.groupByToSet(Data::getName, Data::getValue)
                    .map(GroupBy::toMap).id("results");
        });
    ...
    public void groupByTest() {
        ...
        sep(c -> DataFlowBuilder.subscribe(KeyedData.class)
    ...
    public void groupByFromMapTest() {
        Map<String, Integer> expected = new HashMap<>();
        sep(c ->
                DataFlowBuilder.groupByFromMap(IntegerMap::getMap)
                        .map(GroupBy::toMap)
                        .id("results"));
    ...
    public void joinGroupByTestSimple() {
        ...
        sep(c -> {
            groupBy(KeyedData::getId, KeyedData::getAmount)
                    .innerJoin(DataFlowBuilder.groupBy(String::toString))
                    .mapValues(EventStreamBuildTest::mergedTypeFromTuple)
                    .map(GroupBy::toMap)
                    .sink("merged");
        });
    ...
    public void biMapKeyedItemFromAnotherStreamTest() {
        ...
        assertThat(getStreamed("rs"), is(nullValue()));

        onEvent(new Data("B", 5));
        expected.put("B", new KeyedData("B", 50));
        expected.put("A", new KeyedData("A", 400));
        assertThat(getStreamed("rs"), is(expected));
        assertThat(getStreamed("value"), is(new KeyedData("B", 50)));
    }
    ...
    public void maintainModel() {
        ...
        assertThat(getStreamed("results"), is(nullValue()));
        ...
        assertThat(getStreamed("results"), is(expected));
    ...
    public void abstractAggregateFlowFunctionTest() {
        ...
        sep(c -> {
            DataFlowBuilder.groupBy(Data3::getName, Data3ToSum::new)
        ...
        onEvent(new Data3("A", 10, 1));
        resultMap.put("A", 1);
        assertThat(getStreamed("results"), is(resultMap));
    ...
    public void multipleJoinsTheTupleMapThenReduceTest() {
        ...
        assertThat(getStreamed("pnl"), CoreMatchers.is(Double.NaN));
        onEvent(new MidPrice("GBPUSD", 1.2));
        onEvent(new MidPrice("EURUSD", 1.5));
        assertThat(getStreamed("pnl"), CoreMatchers.is(Double.NaN));

        onEvent(new MidPrice("USDJPY", 100));
        assertThat(getStreamed("pnl"), is(closeTo(-1600, 0.01)));
    }
}
