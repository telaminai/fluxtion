 ...
 */
package com.fluxtion.dataflow.runtime.partition;
...
/**
 * An DataFlow partitioner based upon a received event.A partitioner
 * creates an instance of an {@link DataFlow} and dispatches events
 ...
 * In this case the DataFlow can be partitioned on vehicle
 * identification number.
 * <p>
 * <p>
 * The DataFlow instance will be re-used or a new one created when
 ...
 * manage DataFlow instances in an underlying map. If no key/value
 * mapping is found then a new DataFlow is created and handles the
 * incoming message.
 * <p>
 * <p>
 * New instances are created with s {@link Supplier} node. Optionally an
 * initialiser can be provided that can access the newly created
 * DataFlow before any messages are processed. Using the car/fuel
 ...
 * @author gregp
 */
public class Partitioner<E extends DataFlow> implements DataFlow, Lifecycle, BatchHandler {
    ...
    private DataFlow[] handlerArray;
    private BatchHandler[] batchHandArray;
    private final Supplier<E> factory;
    private Consumer<E> initialiser;

    /**
     * Create a partitioner with a node and initialiser function.
     *
     * @param factory     node creating instances of EventHandlers
    ...
    /**
     * Create a partitioner with a node.
     *
     * @param factory node creating instances of EventHandlers
     */
     ...
     * <li>[CharSrquence] - creates an DataFlow keyed with this
     * CharSequence
     * </ul>
     *
     * @param <I>
    ...
    public void onEvent(Object e) {
        ...
        if (f != null) {
            DataFlow handler = handlerMap.computeIfAbsent(f.apply(e), (t) -> {
                return initialise();
            });
            pushEvent(handler, e);
        }
        if (multiF != null) {
            DataFlow handler = handlerMap.computeIfAbsent(multiF.generateKey(e), (t) -> {
                multiF.newValueList();
                return initialise();
            });
            pushEvent(handler, e);
        ...
        if (!filtered) {
            for (DataFlow eventHandler : handlerArray) {
    ...
    private void pushEvent(DataFlow handler, Object e) {
        handler.onEvent(e);
    }
