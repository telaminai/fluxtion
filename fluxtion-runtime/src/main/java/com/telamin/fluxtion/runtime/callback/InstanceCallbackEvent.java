...
public abstract class InstanceCallbackEvent extends NamedNodeSimple {
    ...
    private static final ThreadLocal<List<Class<? extends InstanceCallbackEvent>>> cbClassList =
...
//        if (GeneratorNodeCollection.service().buildTime()) {
        Class<? extends InstanceCallbackEvent> cbClass = InstanceCallbackEvent.cbClassList.get().remove(0);
...
//        return null;
    ...
    public static void reset() {
        List<Class<? extends InstanceCallbackEvent>> cbClassList = InstanceCallbackEvent.cbClassList.get();
        cbClassList.clear();
