...
import com.telamin.fluxtion.runtime.service.ServiceRegistry;
 ...
 * and methods annotated with {@link OnEventHandler}. An subclass of a DataFlow is
 ...
 *
 * <ul>
 * <li>DataFlow process events of multiple type in a predictable order</li>
 * <li>Application classes are nodes managed by DataFlow and are notified in a predictable manner when an event is processed</li>
 ...
 * <li>All node instances are created and managed by DataFlow</li>
 * </ul>
 * starting point of event dispatch
 *
 * @author Greg Higgins
 */
public interface DataFlow extends ServiceRegistry, NodeDiscovery, Lifecycle {

    @FluxtionIgnore
    DataFlow NULL_EVENTHANDLER = e -> {
    };
     ...
     * see the context when their {@link Initialise} annotated methods are invoked
     */
    default void setContextParameterMap(Map<Object, Object> newContextMapping) {
        throw new UnsupportedOperationException("this DataFlow does not accept updated context map");
    }
     ...
     * <p>
     * Can also be accessed via {@link DataFlowContext#getInjectedInstance(Class)}
     * Can also be accessed via {@link DataFlowContext#getInjectedInstanceAllowNull(Class)}
     *
     ...
     * inject an instance into the running instance with a name qualifier, is available via:
     * {@link InstanceSupplier}.
     * Set the injected type supplied to the CloneableDataFlow
     * <p>
     * Can also be accessed via {@link DataFlowContext#getInjectedInstance(Class)}
     * Can also be accessed via {@link DataFlowContext#getInjectedInstanceAllowNull(Class)}
     *
     ...
     * Set the injected type supplied to the CloneableDataFlow
     * <p>
     * Can also be accessed via {@link DataFlowContext#getInjectedInstance(Class, String)}
     * Can also be accessed via {@link DataFlowContext#getInjectedInstanceAllowNull(Class, String)}
     *
     * @param instance    the instance to inject
    ...
    default void addContextParameter(Object key, Object value) {
        throw new UnsupportedOperationException("this DataFlow does not accept updates to context map");
    }
    ...
    /**
     * Runs a graph calculation cycle invoking any {@link OnTrigger} methods whose
    ...
    /**
     * Type-safe, soft-failing sibling to {@link #getNodeById(String)} —
     * returns an {@link java.util.Optional} of the node cast to the
     * requested type when both the lookup and the cast succeed, otherwise
     * empty.
     *
     * <p>Probes both lookup paths so callers don't have to know which one
     * applies: interpreter SEPs expose registered nodes through
     * {@code nodeNameLookup}, compiled SEPs route auditors through a
     * separate {@link #getAuditorById(String)} reflection lookup. The
     * dual-path workaround lives here, in one place, rather than at every
     * call site.
     *
     * <p>Replaces the typical {@code try/catch + instanceof + cast} dance.
     * Use when the caller is asking "does this node expose interface
     * {@code T}?" rather than "I know this node is registered, what's its
     * value?".
     *
     * @param id   registered node id; must not be {@code null}
     * @param type interface or class to cast to; must not be {@code null}
     * @param <T>  the requested type
     * @return the node cast to {@code T}, or empty if the id is unknown
     *         or the node does not implement {@code T}
     */
    @Preview
    default <T> java.util.Optional<T> getServiceById(String id, Class<T> type) {
        Object node = null;
        try {
            node = getNodeById(id);
        } catch (NoSuchFieldException firstMiss) {
            // Fall through to auditor probe.
        }
        if (node == null) {
            try {
                node = getAuditorById(id);
            } catch (NoSuchFieldException | IllegalAccessException secondMiss) {
                return java.util.Optional.empty();
            }
        }
        return type.isInstance(node) ? java.util.Optional.of(type.cast(node)) : java.util.Optional.empty();
    }

    /**
     * Returns this SEP's {@link com.telamin.fluxtion.runtime.service.ServiceRegistryQuery}
     * — the read-only view of {@code @ServiceRegistered} dependencies declared
     * inside the processor. Empty when the SEP has no
     ...
     *
     * <p>Recommended over a raw node-by-id lookup — callers depend only on
     * the public query interface, not the implementation node class.
     */
    @Preview
    default java.util.Optional<com.telamin.fluxtion.runtime.service.ServiceRegistryQuery> serviceRegistryQuery() {
        return getServiceById(
                com.telamin.fluxtion.runtime.service.ServiceRegistryNode.NODE_NAME,
                com.telamin.fluxtion.runtime.service.ServiceRegistryQuery.class);
    }
    ...
    /**
     * Attempts to get the last {@link LogRecord} as a String if one is available. Useful
     ...
     * @return The {@link DataFlow} cast to an interface
     */
    @SuppressWarnings("unchecked")
