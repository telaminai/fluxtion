...
public class GroupByKey<T> {
    ...
    /**
     * Closed-world constructor — {@code name} and {@code valueClass} are resolved
     * by the generator at build time, so this path performs no method-reference
     * introspection ({@code SerializedLambda}) on the accessors and is compatible
     * with native-image / TeaVM-WASM. {@code accessorsMethods} (used only by the
     * build-time {@code keyPresent} check) is left empty.
     */
    public GroupByKey(List<LambdaReflection.SerializableFunction<T, ?>> accessors, String name, Class<T> valueClass) {
