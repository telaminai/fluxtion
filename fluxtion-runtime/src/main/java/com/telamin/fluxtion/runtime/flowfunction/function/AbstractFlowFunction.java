...
public abstract class AbstractFlowFunction<T, R, S extends FlowFunction<T>> extends EventLogNode
    ...
    public AbstractFlowFunction(
        ...
        if (methodReferenceReflection != null && methodReferenceReflection.captured().length > 0 && !methodReferenceReflection.isDefaultConstructor()) {
            Object streamFunctionInstance = GeneratorNodeCollection.service().addOrReuse(methodReferenceReflection.captured()[0]);
    ...
    /**
     * Closed-world constructor. The {@link MethodReferenceInfo} metadata is
     * resolved by the generator at build time, so this path performs no
     * {@link MethodReferenceReflection#serialized()} reflection (and no
     * {@code GeneratorNodeCollection} / {@code ServiceLoader} access) — making
     * the generated artefact compatible with native-image and TeaVM/WASM.
     */
    @SuppressWarnings("unchecked")
    public AbstractFlowFunction(
            @AssignToField("inputEventStream") S inputEventStream,
        ...
        if (methodReferenceInfo.isStateful()) {
            this.resetFunction = (Stateful<R>) methodReferenceInfo.getResetReference();
        }
    }
