...
public class FileEventFeed extends BaseEventFeed<String> {
    ...
    public void onStart() {
        ...
        log.info(() -> String.format("start FileEventSource %s file:%s tail:%b once:%b, commitRead:%b latestRead:%s readStrategy:%s", roleName(), filename, tail, once, commitRead, latestRead, readStrategy));
        ...
        if (readStrategy == ReadStrategy.ONCE_EARLIEST | readStrategy == ReadStrategy.EARLIEST) {
            ...
            log.info(() -> String.format("%s reading committedReadFile:%s, streamOffset:%s", roleName(), committedReadFile.getAbsolutePath(), streamOffset));
        } else if (commitRead) {
            commitPointer = IoUtil.mapNewFile(committedReadFile, 1024);
            streamOffset = 0;
            log.info(() -> String.format("%s creating committedReadFile:%s, streamOffset:%s", roleName(), committedReadFile.getAbsolutePath(), streamOffset));
        }
    ...
    public int doWork() {
        ...
        try {
            ...
            while (reader.ready()) {
                ...
                int _nread = nread;
                log.finest(() -> String.format("Read %s bytes from %s", _nread, getFilename()));
                ...
                if (nread > 0) {
                    ...
                    do {
                        ...
                        if (line != null) {
                            String _line = line;
                            log.finest(() -> String.format("Read:%s -> %s", filename, _line));
                            readCount++;
                            int _readCount = readCount;
                            log.finest(() -> String.format("Read a line from '%s' count:'%s' line:'%s'", getFilename(), _readCount, _line));
                            if (latestRead) {
                                lastReadLine = line;
                            } else {
                                _publish(line);
    ...
    private Reader connectReader() {
        if (stream == null && filename != null && !filename.isEmpty()) {
            try {
                ...
                log.info(() -> String.format("Opened %s for reading offset %s", getFilename(), streamOffset));
                loggedMissing = false;
